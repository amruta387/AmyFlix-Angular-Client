...
export interface Movie {
  ...
  title: string;
  release_year: number;
  genre: Genre;  // Genre is now an object
  director: Director;  // Director is now an object
  poster?: string;
  featured: boolean;
  director_bio: string;
}
...
export class FetchApiDataService {
  ...
  getUser(username: string): Observable<any> {
    return this.http.get(`${this.apiUrl}/users/${username}`, { headers: this.getAuthHeaders() })
      .pipe(catchError(this.handleError));
  }

  // Get Favorite Movies for a User
  getFavoriteMovies(username: string): Observable<any> {
    return this.http.get(`${this.apiUrl}/users/${username}/movies`, { headers: this.getAuthHeaders() })
      .pipe(catchError(this.handleError));
  }

  // Add Movie to Favorite Movies
  addFavoriteMovie(username: string, movieId: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/users/${username}/movies/${movieId}`, {}, { headers: this.getAuthHeaders() })
      .pipe(catchError(this.handleError));
  }
  ...
  deleteUser(username: string): Observable<any> {
    return this.http.delete(`${this.apiUrl}/users/${username}`, { headers: this.getAuthHeaders() })
      .pipe(catchError(this.handleError));
  }

  // Remove a Movie from Favorite Movies
  removeFavoriteMovie(username: string, movieId: string): Observable<any> {
    return this.http.delete(`${this.apiUrl}/users/${username}/movies/${movieId}`, { headers: this.getAuthHeaders() })
      .pipe(catchError(this.handleError));
  }
